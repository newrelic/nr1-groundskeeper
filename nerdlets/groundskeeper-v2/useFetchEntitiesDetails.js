...
const useFetchEntitiesDetails = ({guidsToFetch = []}) => {
  ...
  useEffect(() => {console.log('GOT DETAILS', detailsData)
    ...
    const detailsObject = entitiesDetails.reduce((acc, {guid, language, applicationInstances}) => ({
      ...acc,
      [guid]: applicationInstances ? entityDetails(applicationInstances, language) : {}
    }), {});
...
const entityDetails = (applicationInstances = [], language) => {
  const { versions, runtimeTypes, features } = applicationInstances.reduce((acc, applicationInstance) => {
    const {agentSettingsAttributes, environmentAttributes} = applicationInstance;
    acc.features = Object.keys(featuresList).reduce((fo, feat) => !acc.features[feat] && agentSettingsAttributes.find(({attribute}) => attribute && attribute === featuresList[feat]) ? ({...fo, [feat]: true}) : fo, acc.features);
    if (!environmentAttributes || !(language in AGENTS_REGEX_STRING)) return acc;
    const versionRegexString = AGENTS_REGEX_STRING[language];
    const foundVersion = environmentAttributes.find(({attribute}) => versionRegexString.test(attribute));
    ...
    if (runtimeType && acc.runtimeTypes.every(r => r !== runtimeType)) acc.runtimeTypes.push(runtimeType);

    return acc;
  }, {versions: [], runtimeTypes: [], features: {dtEnabled: false, infTraceHost: false, logEnabled: false}});

  const display = versions.length === 1 ? versions[0] : versions.join(', ');
  const type = runtimeTypes.length === 1 ? runtimeTypes[0] : runtimeTypes.join(', ');

  return {
    runtimeVersions: { versions, display, type, default: versions.length === 1 ? versions[0] : null },
    features
  };
}
...
const parseRuntimeType = (language, value) => {
  if (language === AGENTS.DOTNET) {
    return value.match(RUNTIMES.DOTNET_CORE.MATCH) ? RUNTIMES.DOTNET_CORE.DISPLAY : RUNTIMES.DOTNET_FRAMEWORK.DISPLAY;
  } else if (language === AGENTS.RUBY) {
    return value.match(RUNTIMES.RUBY_JRUBY.MATCH) ? RUNTIMES.RUBY_JRUBY.DISPLAY : RUNTIMES.RUBY_CRUBY.DISPLAY;
  }
}
